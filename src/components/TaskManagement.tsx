...
export const TaskManagement: React.FC<TaskManagementProps> = ({
  ...
  const calculateFairAssignment = () => {
    ...
    const memberWorkloads = members.map((member) => {
      const memberTasks = tasks.filter(
        (task) => task.assignedTo === member.id && !task.completed
      );
  ...
  const currentMember = members.find(
    (m) => m.name.toLowerCase() === user.username
  );
      ...
      if (task.completed && !scheduledRef.current[task.id]) {
        ...
        if (now >= next5am) {
          ...
          const newTask = {
            ...
            completed: false,
            dueDate: new Date(
              next5am.getTime() + (task.duration || 1) * 24 * 60 * 60 * 1000
            ),
          };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {tasks.map((task) => {
          ...
          const isAssignedToCurrentUser =
            member && user && member.name.toLowerCase() === user.username;
              ...
              <CardContent>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    {member && (
                      <div className="flex items-center space-x-2">
                        <div
                          className={`w-6 h-6 ${member.color} rounded-full flex items-center justify-center text-white text-xs`}
                        >
                          {member.avatar}
                        </div>
                ...
                <Button
                  ...
                  title={
                    canMarkComplete
                      ? undefined
                      : "Only the assigned user or an admin can mark this task complete"
                  }
                >
                ...
                {!canMarkComplete && (
                  <div className="text-xs text-gray-400 text-center mt-1">
                    Only the assigned user or an admin can mark this task
                    complete
                  </div>
                )}
              </CardContent>
        ...
        <DialogContent>
          ...
          {editingTask && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("duration", parseInt(e.target.value))
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("weight", parseInt(e.target.value))
                    }
                  />
                </div>
              </div>
                ...
                <Input
                  ...
                  value={
                    editingTask.dueDate instanceof Date
                      ? editingTask.dueDate.toISOString().slice(0, 10)
                      : editingTask.dueDate
                  }
                  onChange={(e) =>
                    handleEditChange("dueDate", new Date(e.target.value))
                  }
                />
              </div>
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    handleEditChange("assignedTo", value)
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <Button onClick={saveTaskEdits} className="w-full">
                Save Changes
              </Button>
            </div>
          )}
